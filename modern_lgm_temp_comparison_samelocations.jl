...
scatter(mean_ys_mod, mean_y_os_mod,xlims=(-3,25), ylims=(-3,25), 

        xlabel="Mean Surface Temperature \$\\bar{\\theta}_s(^\\circ C)\$", 
        ylabel="Mean Ocean Temperature \$\\bar{\\theta}(^\\circ C)\$", 
        title="LGM vs. Mod. Ocean Temp.", color=:darkcyan,
        label="Mean (20 mod. samples)",marker=(:diamond, 3),
        markerstrokewidth=0.3,opacity=0.5,legend=:topleft,markersize=3,
        legendfontsize=8, dpi=1000)
scatter!(mean_ys_lgm, mean_y_os_lgm, 
        label="Mean (20 LGM samples)",color=:darkmagenta,
         markerstrokewidth=0.3,opacity=0.3,
         legendfontsize=8)
...
# scatter!(y_hot20_lgm, y_o_hot20_lgm, label="Hot 100 LGM points",markerstrokewidth=0,
#         legendfontsize=8)
# scatter!(y_hot20_mod, y_o_hot20_mod, label="Hot 100 Mod. points",marker=(:diamond, 3),markerstrokewidth=0,
#         legendfontsize=8)

# scatter!(y_cool20_lgm, y_o_cool20_lgm, label="Cool 100 LGM points",markerstrokewidth=0,
#         legendfontsize=8)
...
scatter!([mean_y_all_mod], [mean_y_o_all_mod], 
         ...
         marker=(:diamond, 12), markerstrokewidth=2, opacity=0.9,
        legendfontsize=8)
scatter!([mean_y_all_lgm], [mean_y_o_all_lgm], 
        label="True mean LGM ($mean_y_all_lgm (\$\\bar{\\theta}_s\$) and $mean_y_o_all_lgm (\$\\bar{\\theta}\$))",
        markerstrokewidth=2, markersize=12,
        legendfontsize=8)



savefig("Sampling_samegrid_5.png",)
