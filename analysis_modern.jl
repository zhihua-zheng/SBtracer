...
function bootstrap_stats(TMIversion, γ, N, B)
    ...
    for i in 1:Bootst
        y, _, _, _, _, _ = TMI.surface_synthetic_observations(TMIversion, "θ", γ, N)
        y_o, _, _, _, _, _ = TMI.ocean_synthetic_observations(TMIversion, "θ", γ, N)
...
y_all, _, _, _, _, _ = TMI.surface_synthetic_observations(TMIversion, "θ", γ, N_surf)
y_o_all, _, _, _, _, _ = TMI.ocean_synthetic_observations(TMIversion, "θ", γ, N_ocean)

n = 1000;
y_hot20 = sort(y_all, rev=true)[1:n]
y_o_hot20 = sort(y_o_all, rev=true)[1:n]

y_cool20 = sort(y_all)[1:n]
y_o_cool20 = sort(y_o_all)[1:n]
...
scatter(mean_ys, mean_y_os, xlims=(-3,30), ylims=(-3,30), 
        ...
        title="Scatter Plot of Mean Temperatures", 
